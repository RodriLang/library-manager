...
public class InventoryCountService {
    ...
    @Transactional(readOnly = true)
    public Page<InventoryCountItemResponse> findItems(Long sessionId, Pageable pageable) {
        accessService.require(sessionId);
        return itemRepository.findAllBySessionId(sessionId, pageable).map(responseMapper::toItemResponse);
    }

    @Transactional(readOnly = true)
    public Page<InventoryCountResultResponse> findResults(Long sessionId, Pageable pageable) {
        accessService.require(sessionId);
        return resultRepository.findAllBySessionIdAndCountedQuantityIsNotNull(sessionId, pageable)
                .map(responseMapper::toResultResponse);
    }
