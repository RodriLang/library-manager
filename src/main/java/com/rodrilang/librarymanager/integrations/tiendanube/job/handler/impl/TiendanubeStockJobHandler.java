...
public class TiendanubeStockJobHandler implements TiendanubeJobHandler {
    ...
    public void execute(TiendanubeJobExecutionContext context) {
        ...
        if (prepared.isEmpty()) {
            log.info("Stock sync job omitted because the inventory is no longer linked. inventoryId={}", context.inventoryId());
            return;
        }
        ...
        try {
            client.updateStock(snapshot.storeId(), snapshot.productId(), snapshot.variantId(), snapshot.stock());
            resultService.registerLinkedSuccess(snapshot);

            log.info("Stock sincronizado con Tiendanube. inventoryId={} stock={}", snapshot.inventoryId(), snapshot.stock());
